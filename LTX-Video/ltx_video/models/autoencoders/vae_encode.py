...
def normalize_latents(
    ...
    return (
        (latents - vae.mean_of_means.to(dtype=latents.dtype, device=latents.device).view(1, -1, 1, 1, 1))
        / vae.std_of_means.to(dtype=latents.dtype, device=latents.device).view(1, -1, 1, 1, 1)
...
def un_normalize_latents(
    ...
    return (
        latents * vae.std_of_means.to(dtype=latents.dtype, device=latents.device).view(1, -1, 1, 1, 1)
        + vae.mean_of_means.to(dtype=latents.dtype, device=latents.device).view(1, -1, 1, 1, 1)
        if vae_per_channel_normalize
        else latents / vae.config.scaling_factor
    )
