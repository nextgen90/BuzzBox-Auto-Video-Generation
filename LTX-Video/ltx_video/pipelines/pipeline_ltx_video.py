...
class LTXVideoPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        **kwargs,
        ...
        if prompt_embeds is None:
            ...
            # Delete temporary input tensors to free CPU RAM/VRAM
            del text_input_ids
            del untruncated_ids
            del text_inputs
        ...
        if do_classifier_free_guidance:
            ...
            # Delete temporary uncond inputs
            del uncond_input
        else:
    ...
    def prepare_latents(
        ...
        if media_items is not None:
            media_items_gpu = media_items.to(dtype=self.vae.dtype, device=self.vae.device)
            latents = vae_encode(
                media_items_gpu,
                self.vae,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Immediately clean up CPU/GPU memory after text encoding
        import gc
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        ...
        # Caches to prevent repeated allocations and memory fragmentation during denoising
        fractional_coords_cache = {}
        conditioning_mask_cache = {}
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                # Use caches for constant loop tensors based on num_conds
                if num_conds not in fractional_coords_cache:
                    batch_pixel_coords = torch.cat([pixel_coords] * num_conds)
                    fractional_coords = batch_pixel_coords.to(dtype=torch.float32, device=device)
                    fractional_coords[:, 0] = fractional_coords[:, 0] * (1.0 / frame_rate)
                    fractional_coords_cache[num_conds] = fractional_coords
                else:
                    fractional_coords = fractional_coords_cache[num_conds]

                conditioning_mask = orig_conditioning_mask
                if conditioning_mask is not None and is_video:
                    assert num_images_per_prompt == 1
                    if num_conds not in conditioning_mask_cache:
                        conditioning_mask_cache[num_conds] = torch.cat([conditioning_mask] * num_conds)
                    conditioning_mask = conditioning_mask_cache[num_conds]
                ...
                if conditioning_mask is not None:
                    ...
                    if num_conds not in one_minus_conditioning_mask_cache:
                        one_minus_conditioning_mask_cache[num_conds] = 1.0 - conditioning_mask
                    current_timestep = torch.min(
                        current_timestep, one_minus_conditioning_mask_cache[num_conds]
                    )
    ...
    def prepare_conditioning(
        ...
        if conditioning_items:
            ...
            for conditioning_item in conditioning_items:
                ...
                # Encode the provided conditioning media item on GPU, then clean up
                media_item_gpu = media_item.to(dtype=self.vae.dtype, device=self.vae.device)
